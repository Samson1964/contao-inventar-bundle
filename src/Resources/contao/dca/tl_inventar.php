...
(
	...
	(
		...
		'adressen' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_inventar']['adressen'],
			'exclude'                 => true,
			...
			(
				...
				(
					'public_plzort' => array
					(
						'label'                 => &$GLOBALS['TL_LANG']['tl_inventar']['adressen_public_plzort'],
						'exclude'               => true,
						'inputType'             => 'checkbox',
						'eval'                  => array
						(
					...
					'plz' => array
					(
						'label'                 => &$GLOBALS['TL_LANG']['tl_inventar']['adressen_plz'],
						'exclude'               => true,
						'inputType'             => 'text',
						'eval'                  => array
						(
					...
					'ort' => array
					(
						'label'                 => &$GLOBALS['TL_LANG']['tl_inventar']['adressen_ort'],
						'exclude'               => true,
						'inputType'             => 'text',
						'eval'                  => array
						(
					...
					'public_str' => array
					(
						'label'                 => &$GLOBALS['TL_LANG']['tl_inventar']['adressen_public_str'],
						'exclude'               => true,
						'inputType'             => 'checkbox',
						'eval'                  => array
						(
					...
					'strasse' => array
					(
						'label'                 => &$GLOBALS['TL_LANG']['tl_inventar']['adressen_strasse'],
						'exclude'               => true,
						'inputType'             => 'text',
						'eval'                  => array
						(
					...
					'googlemap' => array
					(
						'label'                 => &$GLOBALS['TL_LANG']['tl_inventar']['adressen_googlemap'],
						'exclude'               => true,
						'inputType'             => 'checkbox',
						'eval'                  => array
						(
		...
		'telefone' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_inventar']['telefone'],
			'exclude'                 => true,
			'inputType'               => 'multiColumnWizard',
			'eval'                    => array
			(
				...
				(
					'public_num' => array
					(
						'label'                 => &$GLOBALS['TL_LANG']['tl_inventar']['telefone_public_num'],
						'exclude'               => true,
						'inputType'             => 'checkbox',
						'eval'                  => array
						(
					...
					'nummer' => array
					(
						'label'                 => &$GLOBALS['TL_LANG']['tl_inventar']['telefone_nummer'],
						'exclude'               => true,
						'inputType'             => 'text',
						'eval'                  => array
						(
...
{
	$GLOBALS['TL_DCA']['tl_inventar']['fields']['singleSRC']['sql'] = "binary(16) NULL";
}
...
class tl_inventar extends Backend
{
	...
	public function generateSearchstring(DataContainer $dc)
	{
		...
		\Database::getInstance()->prepare("UPDATE tl_inventar SET searchstring = ? WHERE id = ?")
		                        ->execute($temp, $dc->id);
	}
	...
	public function generateAdvancedFilter(\DataContainer $dc)
	{
		...
		(
			...
			(
				'name'    => 'adr_filter',
				'label'   => $GLOBALS['TL_LANG']['tl_inventar']['filter_extended'],
				'options' => array
				(
					'1'  => $GLOBALS['TL_LANG']['tl_inventar']['filter_emaildoubles'],
				)
			),
		);
...
<strong>' . $GLOBALS['TL_LANG']['tl_inventar']['filter'] . ':</strong> ' . "\n";
...
<option value="' . $arrFilter['name'] . '">---</option>' . "\n";
			...
			{
				$strOptions .= '  <option value="' . $k . '"' . (($session['filter']['tl_inventar_filter'][$arrFilter['name']] === (string) $k) ? ' selected' : '') . '>' . $v . '</option>' . "\n";
			}

			$strBuffer .= '<select name="' . $arrFilter['name'] . '" id="' . $arrFilter['name'] . '" class="tl_select' . (isset($session['filter']['tl_inventar_filter'][$arrFilter['name']]) ? ' active' : '') . '">
' . $strOptions . '
</select>' . "\n";
		}
	...
	public function applyAdvancedFilter()
	{
		...
		foreach($_POST as $k => $v)
		{
			...
			if($k == \Input::post($k))
			{
				unset($session['filter']['tl_inventar_filter'][$k]);
			} 
			// Filter zuweisen
			else
			{
				$session['filter']['tl_inventar_filter'][$k] = \Input::post($k);
			}
		}

		$this->Session->setData($session);

		if(\Input::get('id') > 0 || !isset($session['filter']['tl_inventar_filter']))
		{
			return;
		}
		...
		switch($session['filter']['tl_inventar_filter']['adr_filter'])
		{
			case '1': // Adressen mit doppelten E-Mail-Adressen anzeigen
				$objAdressen = \Database::getInstance()->prepare("SELECT * FROM tl_inventar")
				                                       ->execute();
		...
		$GLOBALS['TL_DCA']['tl_inventar']['list']['sorting']['root'] = $arrAdressen;
